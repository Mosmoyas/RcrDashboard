...
const Friends = () => {
  const FriendsData = DataFriends.map((friend, index) => {
    return (
      <>
        <div
          key={index}
          className="friend bg-white p-20 rad-6 p-relative"
          id={friend.id}
        >
          ...
          <div className="icons fs-13 p-relative">
            ...
            <span className="vip fw-bold c-orange">
              {friend.vip === true ? "VIP" : " "}
            </span>
          </div>
  ...
  return (
    <>
      ...
      <div className="friends-page d-grid m-20 gap-20">{FriendsData}</div>
    </>
  );
};
